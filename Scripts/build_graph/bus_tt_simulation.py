    ...
    for this_bus,events in BUS_EVENTS.iteritems():  
        ...
        for this_event_tup in events:     
            ...
            if last_event > 0 and cont == 0:
                edge_list.append((event_id[str(last_event)],\
                    event_id[str(this_event)],this_time-last_time))
    ...
    for this_stop,events in events_at_stop.iteritems():              
        ...
        if len(events) > 1:   
            ...
            for this_time in events:   
                if int(this_time) < 6*3600 or int(this_time) > 9*3600:
                    continue
                this_event = event_id["s"+str(this_stop)+"t"+str(this_time)]
                ...
                if False:
                    ...
                    for tup in nearby_stops[this_stop]:
                        ...
                        next_event_code = "s"+str(nearby_stop)+"t"+str(this_time+walk_time)
                                            
                        #add walk links to graph
                        if next_event_code not in event_id:
                            event_id[next_event_code] = i
                            i+=1                          
                        next_event = event_id[next_event_code]
                ...
                if last_time != 0 and int(this_time) >= 6*3600 and int(this_time) <= 9*3600:
                    last_event = event_id["s"+str(this_stop)+"t"+str(last_time)]
...
def getShortestPath(start_node,end_node):
    ...
    nx.draw_networkx_edges(GRAPH,EVENT_POS,edgelist=path_edges,edge_color='b',
                           width=5)
    nx.draw_networkx_nodes(GRAPH,EVENT_POS,nodelist=[start_node,end_node],
                           node_size=60,node_color ='b')
